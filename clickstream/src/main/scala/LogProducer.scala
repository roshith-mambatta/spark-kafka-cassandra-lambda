...
object LogProducer extends App {
  ...
  props.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, wlc.kafkaServer)
  ...
  for (fileCount <- 1 to wlc.numberOfFiles) {
    ...
    for (iteration <- 1 to wlc.records) {
      ...
      val jsonString = write(ClickEvent(adjustedTimestamp,referrer,action,prevPage,visitor,page,product))

      //send to kafka
      val producerRecord = new ProducerRecord(topic, jsonString)
